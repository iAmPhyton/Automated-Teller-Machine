...
allowedUsers = ['Gemma', 'Ella', 'Jane']
acctBalance = [100, 1900, 300]
allowedPassword = ['random4digitNumber-1', 'random4digitNumber-2', 'random4digitNumber-3']
...
def userLogin():
    ...
    while login == True:
        ...
        if(name in allowedUsers):
            ...
            while cnt == True:    # Conditional to check if the user wants to quit
                ...
                if(password == allowedPassword[namePosition]):
                    ...
                    while True:
                        ...
                        print('Available actions: on %d' %
                              acctNumbers[namePosition])
                        print(
                            '1. Withdrawal\n2. Cash Deposit\n3. Check Balance\n4. Change Password\n5.Transfer\n6. Complaint')
                        ...
                        if choice == '1':
                            ...
                            acctBalance[namePosition] = acctBalance[namePosition] - \
                                withdrawAmount
                            print('Current Balance: %g' %
                                  (acctBalance[namePosition]))
                            # Write data to drive
                            shelFile = shelve.open('sysdata')
                            shelFile['Balance'] = acctBalance
                            shelFile.close()
                            ...
                            if newPwd == confirmatory:
                                allowedPassword[allowedUsers.index(
                                    name)] = newPwd
                            else:
                            ...
                            print('You seleceted %s' % choice)
                            response = input(
                                "Enter recipients' account number or username\n")
                            amt = int(input('Enter the amount to transfer'))
                            if (response in allowedUsers):
                                print('You have transferred %g to %s' %
                                      (amt, response))
                                acctBalance[allowedUsers.index(
                                    response)] = acctBalance[allowedUsers.index(response)] + amt
                                # Write data to drive
                                shelFile = shelve.open('sysdata')
                                shelFile['Balance'] = acctBalance
                                shelFile.close()
                                ...
                                print('You have transferred %g to %d' %
                                      (amt, response))
                                acctBalance[namePosition] = acctBalance[namePosition] - amt
                                acctBalance[acctNumbers.index(
                                    response)] = acctBalance[acctNumbers.index(response)] + amt
                                # Write data to drive
                                shelFile = shelve.open('sysdata')
                                shelFile['Balance'] = acctBalance
                                shelFile.close()
                            ...
                            response = input(
                                'Invalid option selected, to try again press 1 , to exit 2: ')
                            if response == '2':
                                cnt = False
                                sys.exit()
            ...
            response = input(
                'Name not found, press  1 to try again, or 2 to exit: ')
            if response == '2':
                sys.exit()
